...
(* fun solve : con -> subst ,
    takes in a constraint and returns a substitution , which when applied to the
    constraint , satisfies it . If the constraint is unsolvable , it calls
    unsatisfiableEquality .*)

fun solve c = 
    let
    fun solve_constr TRIVIAL              = idsubst
      | solve_constr (constr1 /\ constr2) =
                        let 
                        val theta1 = solve constr1
                        val theta2 = solve (consubst theta1 constr2)
                        in
                        compose (theta2,theta1)
                        end
      | solve_constr (TYVAR tyvar ~ tau)  = 
                          let 
                          fun occurs_check name tau = 
                              member name (freetyvars tau)
                          in
                          case occurs_check tyvar tau of  
                          true => if eqType(TYVAR tyvar,tau)
                                  then idsubst
                                  else raise 
                                  unsatisfiableEquality(TYVAR tyvar,tau)
                           | _ => tyvar |--> tau
                          end
      | solve_constr (tau ~ TYVAR tyvar)    = solve_constr (TYVAR tyvar ~ tau)
      | solve_constr (TYCON tau1 ~ TYCON tau2) = 
                 if eqType(TYCON tau1,TYCON tau2)
                 then idsubst
                 else raise
                 unsatisfiableEquality(TYCON tau1,TYCON tau2)
      | solve_constr (CONAPP (tau1,list1) ~ CONAPP (tau2,list2)) = 
              solve_constr 
              ((ListPair.foldrEq (fn (a,b,c)=> (a ~ b) /\ c) TRIVIAL 
                (list1,list2)) /\ (tau1 ~ tau2))
      | solve_constr (tau1 ~ tau2) = raise unsatisfiableEquality(tau1,tau2) 
    in 
    solve_constr c 
    end 
...
val () = Unit.checkAssert "'a ~ list int  is solved by 'a |--> listtype int"
         (fn () => solutionEquivalentTo (TYVAR "'a" ~ listtype inttype, 
                                         "'a" |--> listtype inttype ))

(*LHS occurs in RHS , unsolvable *)
val () = Unit.checkAssert "'a ~ listtype 'a cannot be solved"
         (fn () => hasNoSolution (alpha ~ listtype alpha))

(*LHS does not occur in RHS ,solvable *)
val () = Unit.checkAssert "'a ~ listtype 'b can be solved"
         (fn () => hasGoodSolution (alpha ~ listtype beta))

(*LHS occurs in RHS , unsolvable *)
val () = Unit.checkAssert "listtype 'a ~ 'a cannot be solved"
         (fn () => hasNoSolution (listtype alpha ~ alpha))


val () = Unit.checkAssert " 'a ~ 'a is solved by idsubst"
         (fn () => solutionEquivalentTo (alpha ~ alpha, 
                                         idsubst))

val () = Unit.checkAssert 
      (constraintString (pairtype(inttype,booltype)~pairtype(inttype,booltype)))
        (fn () => hasGoodSolution 
        (pairtype(inttype,booltype) ~ pairtype(inttype,booltype)) )
...
val () = Unit.checkAssert 
      (constraintString 
      (funtype([inttype,listtype booltype],inttype)~pairtype(inttype,booltype)))
        (fn () => hasNoSolution 
     (funtype([inttype,listtype booltype],inttype)~pairtype(inttype,booltype)))


val () = Unit.checkAssert 
      (constraintString (CONAPP (alpha,[listtype inttype])
          ~CONAPP(beta,[listtype alpha])))
        (fn () => hasGoodSolution (CONAPP (alpha,[listtype inttype])
                  ~CONAPP(beta,[listtype alpha])) )

val () = Unit.checkAssert 
      (constraintString (CONAPP (alpha,[listtype inttype,booltype])
          ~CONAPP(beta,[listtype alpha,inttype])))
        (fn () => hasNoSolution (CONAPP (alpha,[listtype inttype,booltype])
                  ~CONAPP(beta,[listtype alpha,inttype])) )
...
(* function : typeof :exp * type_env -> ty * con 
   accepts a tuple(exp,type_env) ,and returns a tuple (ty,con) containing the 
   resulting  type  of the expression if the constraint has a valid 
   substitution. *)
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 438b *)
      fun literal  (NUM _)   = (inttype,TRIVIAL)
        ...
        | literal  (PAIR(v1,v2)) = 
             let 
              val typv1 = literal v1
              val typv2 = literal v2 
             in (fst typv2,conjoinConstraints 
               [snd typv1,snd typv2,(listtype (fst typv1))~(fst typv2)])
             end 
        | literal   _         = raise BugInTypeInference "sa"
      ...
      fun ty (LITERAL n) = literal n
        ...
        | ty (IFX (e1, e2, e3))        = 
              let
               ...
               val tc2 = ty e2
               val tc3 = ty e3 
               val conj = conjoinConstraints 
                          (List.map (fn (a,b)=>b) [tc1,tc2,tc3])
              in (fst tc2 ,conjoinConstraints 
                   [conj,(fst tc1)~booltype,(fst tc2)~(fst tc3)])
              end 
        | ty (BEGIN es)                = 
            (case es of 
                    []   => (unittype,TRIVIAL)
                 | [x]   => ty x
                 | x::xs => let
                             val tail = ty (BEGIN xs)
                            in 
                            (fst tail,snd (ty x) /\ snd tail)
                            end )
        | ty (LAMBDA (formals, body))  = 
             let
              val newGamma = List.foldl (fn(name,oldGamma)=>
                      bindtyscheme (name, FORALL ([],freshtyvar ()),oldGamma))
                      Gamma formals 
              val exptype = typeof(body,newGamma)
              val args = List.map (fn(name)=> freshInstance 
                          (findtyscheme (name,newGamma))) formals
                        
             in (funtype(args,fst exptype),snd exptype)
             end 
                                                                  
        | ty (LETX (LET, bs, body))    = 
          let val list_typconstr = 
              List.map (fn (name,e) => ty e) bs 
              val conjconstrs = List.foldr (fn (c,conj) => (snd c) /\ conj)
                                   TRIVIAL list_typconstr
              val theta = solve conjconstrs 
              val set_freevars = inter (dom theta,freetyvarsGamma Gamma)
              val new_constraint = List.foldl (fn (alpha,constr)=> 
                               constr /\ (TYVAR alpha ~ 
                                    (tysubst theta (TYVAR alpha)))) 
                               TRIVIAL set_freevars
              val freetys = union(freetyvarsGamma Gamma,freetyvarsConstraint 
                                                         new_constraint)
              val name_list = List.map (fn (name,_) => name) bs
              val newGamma = ListPair.foldrEq (fn (name,typcons,oldGamma)=> 
                              bindtyscheme (name,
                            generalize (tysubst theta (fst typcons),freetys)
                            ,oldGamma))
                            Gamma (name_list,list_typconstr)
              val ty_body = typeof (body,newGamma)
          in (fst ty_body,new_constraint /\ snd ty_body)
          end 

           
        | ty (LETX (LETREC, bs, body)) = 
           let 
            val newGamma = List.foldl (fn((name,_),oldGamma)=>bindtyscheme
                          (name,FORALL ([],freshtyvar()),oldGamma)) Gamma bs 
            val list_typCons =List.map (fn (_,exp) => typeof (exp,newGamma)) bs
            val conj_constrexp = conjoinConstraints 
                                 (List.map (fn(_,constr)=> constr) list_typCons)
            val conjtosolve = ListPair.foldlEq 
                       (fn ((name,exp),(typ,_),constr)
                     => constr /\
                   ((freshInstance (findtyscheme (name,newGamma))) ~ typ))
                   conj_constrexp (bs,list_typCons)
            val theta = solve conjtosolve
            val set_freevars = inter (dom theta,freetyvarsGamma Gamma)
            val C' = TRIVIAL /\ conjoinConstraints (List.map (fn(alpha)=> 
                    TYVAR alpha ~ (tysubst theta (TYVAR alpha)) ) set_freevars)
            val freetys = union(freetyvarsGamma Gamma,freetyvarsConstraint C')
            val Gamma' = ListPair.foldrEq (fn ((name,_),typcons,oldGamma)=> 
                              bindtyscheme (name,
                            generalize (tysubst theta (fst typcons),freetys)
                            ,oldGamma))
                            Gamma (bs,list_typCons)
            val ty_body = typeof (body,Gamma')
            val type_body = fst ty_body
            val Constr_body = snd ty_body
           in 
            (type_body,C' /\ Constr_body)
           end 
